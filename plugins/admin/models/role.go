...
func Role() RoleModel {
	return RoleModel{Base: Base{TableName: "himera_roles"}}
}

// RoleWithId return a default role model of given id.
func RoleWithId(id string) RoleModel {
	idInt, _ := strconv.Atoi(id)
	return RoleModel{Base: Base{TableName: "himera_roles"}, Id: int64(idInt)}
}
...
func (t RoleModel) CheckPermission(permissionId string) bool {
	checkPermission, _ := t.Table("himera_role_permissions").
...
func (t RoleModel) DeletePermissions() error {
	return t.WithTx(t.Tx).Table("himera_role_permissions").
		Where("role_id", "=", t.Id).
		Delete()
}
